...
public class PointModel2dRl extends RootModel
{
   ...
   public StepAdjustment advance(double t0, double t1, int flags)
   {
      ...
      {         
         ...
         try {
            ...
            //print(jo.toString ());
            if (!networkServer.send (jo_send_state))
         ...
         try {
            JSONObject jo_rec = networkServer.receive ();
            if (jo_rec.getString ("type").equals ("excitations")) 
               fillExcitations(jo_rec);
         }
         catch (JSONException e) {
         ...
         try {
            ...
            //print(jo.toString ());
            networkServer.send (jo_send_result);            
         }
         catch (JSONException e)
         {
   ...
   private void fillExcitations (JSONObject jo_rec) throws JSONException
   {
